...
class DistractorGenerator():
    def __init__(self):
        ...
        checkpoint_path = 'ml_app/models/distractors.ckpt'
        self.dg_model = QAModel.load_from_checkpoint(checkpoint_path)
    ...
    def generate(self, generate_count: int, correct: str, question: str, context: str) -> List[str]:
        ...
        if re.search(math_pattern, correct):
           distractors = self.generate_math_distractors(correct)
        else:
    ...
    def generate_math_distractors(self, input: str, num_distractrs=3) -> list[str]:
        distractors = []
        expression =input

        
        for _ in range(num_distractrs):
            # Use regular expressions to find all numbers, math functions, and operators in the expression
            numbers = re.findall(r'[-+]?\d*\.\d+|\d+', expression)
            functions = re.findall(r'[a-zA-Z]+\((.*?)\)', expression)
            operators = re.findall(r'[+\-*/^]', expression)
            # Modify the numbers, math functions, and operators
            new_numbers = [str(random.randint(0, 100)) for _ in numbers]
            new_functions = [random.choice([f.__name__ for f in functions.__class__.__bases__[0].__subclasses__()]) for _ in functions]
            new_operators = [random.choice(['+', '-', '*', '/']) for _ in operators]

            # Replace the original numbers, math functions, and operators with the new ones
            for i, number in enumerate(numbers):
                expression = expression.replace(number, new_numbers[i])
            for i, function in enumerate(functions):
                expression = expression.replace(function, new_functions[i])
            for i, operator in enumerate(operators):
                expression = expression.replace(operator, new_operators[i])
            distractors.append(expression)
