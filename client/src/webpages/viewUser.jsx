...
export default function ProfilePage() {
  ...
  const [currentuser, setCurrUserData] = useState({});
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (isFull && user.id){
          await loadContent(user.id, currentuser.id);
                  // console.log(user.id);
  ...
  const loadContent = async (userId, currentuser) => {
    console.log(userId);
    try {
      const response = await axios.get(`/users/${userId}/posts/public/${currentuser}`, {
        headers: {
          'X-CSRFToken': csrfToken,
          'Authorization': `Bearer ${token}`
        }
              ...
              <div className="card mb-4">
                <div className="card-body">
                  ...
                  <div className='toggle-post'>
                    <h6 id='my-post' onClick={handleShowPosts}>Posts</h6>
